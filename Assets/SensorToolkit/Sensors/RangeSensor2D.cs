    ...
    public class RangeSensor2D : BaseAreaSensor, IPulseRoutine
    {
        ...
        class OverlapCircleTest : ITestNonAlloc<RangeSensor2D, Collider2D>
        {
            public int Test(RangeSensor2D sensor, Collider2D[] results)
            {
                ...
                return Physics2D.OverlapCircle(sensor.transform.position, sensor.Circle.Radius, contactFilter2D,
                    results);
                // return Physics2D.OverlapCircleNonAlloc(sensor.transform.position, sensor.Circle.Radius, results,
        ...
        class OverlapBoxTest : ITestNonAlloc<RangeSensor2D, Collider2D>
        {
            public int Test(RangeSensor2D sensor, Collider2D[] results)
            {
                ...
                return Physics2D.OverlapBox(sensor.transform.position, 2 * sensor.Box.HalfExtents,
                    sensor.transform.eulerAngles.z, contactFilter2D, results);
                // return Physics2D.OverlapBoxNonAlloc(sensor.transform.position, 2 * sensor.Box.HalfExtents,
                //     sensor.transform.eulerAngles.z, results, sensor.DetectsOnLayers);
            }
        }
        ...
        {
            SensorGizmos.PushColor(STPrefs.RangeColour);
            SensorGizmos.PushMatrix(Matrix4x4.TRS(transform.position,
                Quaternion.AngleAxis(transform.eulerAngles.z, Vector3.forward), Vector3.one));
        ...
        class OverlapCapsuleTest : ITestNonAlloc<RangeSensor2D, Collider2D>
        {
            public int Test(RangeSensor2D sensor, Collider2D[] results)
            {
                ...
                return Physics2D.OverlapCapsule(pos, size, dir, angle, contactFilter2D, results);
            }
        }
