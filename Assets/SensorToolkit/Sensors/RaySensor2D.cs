...
namespace Micosmo.SensorToolkit
{
    ...
    public class RaySensor2D : Sensor, IRayCastingSensor, IPulseRoutine
    {
        ...
        [Tooltip(
            "In Collider mode the sensor detects GameObjects attached to colliders. In RigidBody mode it detects the RigidBody GameObject attached to colliders.")]
        ...
        [SerializeField] PulseRoutine pulseRoutine = new PulseRoutine();
        ...
        {
            get { return IgnoreTriggerColliders ? QueryTriggerInteraction.Ignore : QueryTriggerInteraction.Collide; }
        }
        ...
        protected override void OnDrawGizmosSelected()
        {
            ...
            if (ShowDetectionGizmos)
            {
                ...
                if (IsObstructed)
                {
                    SensorGizmos.RaycastHitGizmo(GetObstructionRayHit().Point + depth, GetObstructionRayHit().Normal,
                        true);
                }
            }
        }
        ...
        class CircleCastTest : ITestNonAlloc<RaySensor2D, RaycastHit2D>
        {
            public int Test(RaySensor2D sensor, RaycastHit2D[] results)
            {
                ...
                if (sensor.isSingleResult())
                {
                    var hit = Physics2D.CircleCast(ray.origin, sensor.Circle.Radius, ray.direction, sensor.Length,
                        combinedLayers);
                    if (hit.collider != null)
                    {
                        results[0] = hit;
                ...
                else
                {
                    ...
                    return Physics2D.CircleCast(ray.origin, sensor.Circle.Radius, ray.direction, contactFilter2D,
                        results, sensor.Length);
        ...
        void DrawCircleGizmo()
        {
            ...
            SensorGizmos.CirclecastGizmo(new Ray(transform.position, direction), length, euler, Circle.Radius,
                showObstruction);
        }
        ...
        class BoxCastTest : ITestNonAlloc<RaySensor2D, RaycastHit2D>
        {
            public int Test(RaySensor2D sensor, RaycastHit2D[] results)
            {
                ...
                if (sensor.isSingleResult())
                {
                    var hit = Physics2D.BoxCast(sensor.transform.position, sensor.Box.HalfExtents,
                        sensor.transform.eulerAngles.z, sensor.direction, sensor.Length, combinedLayers);
                    if (hit.collider != null)
                    {
                        results[0] = hit;
                        return 1;
                ...
                else
                {
                    ...
                    return Physics2D.BoxCast(sensor.transform.position, sensor.Box.HalfExtents,
                        sensor.transform.eulerAngles.z, sensor.direction, contactFilter2D, results, sensor.Length);
        ...
        void DrawBoxGizmo()
        {
            ...
            var euler = Quaternion.Euler(0, 0, transform.rotation.eulerAngles.z);
            var direction = WorldSpace ? Quaternion.Inverse(euler) * Direction : (Vector3)Direction;
            SensorGizmos.BoxcastGizmo(new Ray(transform.position, direction), length, euler, Box.HalfExtents,
                showObstruction);
        }
        ...
        class CapsuleCastTest : ITestNonAlloc<RaySensor2D, RaycastHit2D>
        {
            public int Test(RaySensor2D sensor, RaycastHit2D[] results)
            {
                ...
                if (sensor.isSingleResult())
                {
                    var hit = Physics2D.CapsuleCast(pos, size, dir, angle, sensor.direction, sensor.Length,
                        combinedLayers);
                    if (hit.collider != null)
                    {
                        results[0] = hit;
                ...
                else
                {
                    ...
                    return Physics2D.CapsuleCast(pos, size, dir, angle, sensor.direction, contactFilter2D, results,
                        sensor.Length);
        ...
        void DrawCapsuleGizmo()
        {
            ...
            var euler = Quaternion.Euler(0, 0, transform.rotation.eulerAngles.z);
            var direction = WorldSpace ? Quaternion.Inverse(euler) * Direction : (Vector3)Direction;
            SensorGizmos.Capsule2DcastGizmo(new Ray(transform.position, direction), length, euler, Capsule.Radius,
                Capsule.Height, showObstruction);
        }
